...
<?php
    include("config.php");
    if(isset($_GET['corresp'])){
        $_SESSION['correspondant']=$_GET['corresp'];
    }else{
        $_SESSION['correspondant']=$_SESSION['correspondant'];
    } 
    $correspondant=$_SESSION['correspondant'];
    $admins=$dbh->query("SELECT * FROM coc_admin WHERE COC_ADMIN_correspondant='$correspondant'"); 
    $thisadmin=$admins->fetch(); 
    
    $status=$dbh->query("SELECT COC_USER_status FROM coc_user WHERE COC_USER_id=".$thisadmin["COC_USER_id"])->fetch();  
...
*/
    //$iddest=$thisadmin['COC_ADMIN_id']; 
    //$idexp=$_SESSION['client']['id'];
            $idexp=$_SESSION['client']['id'];
            $iddest=$thisadmin['COC_ADMIN_id']; 
            $typedesti=$thisadmin['COC_ADMIN_correspondant'];
            $typeexp=$_SESSION['user']['type'];
    ...
    function getMessages(){ 
      ...
      $resultats = $dbh->query("SELECT * FROM coc_support WHERE idexp='$idexp' and iddest='$iddest' OR idexp='$iddest' and iddest='$idexp'  ORDER BY date DESC LIMIT 10 ");
      return $resultats;
    }
    $req=getMessages();
    foreach  ($req as $row) {
...
<head>
    ...
    />
    <meta name="description" content="Messagerie web">
    ...
    <script  rel="javascript" type="text/javascript" src="assets/scripts/script.js"></script>
...
<script type="text/javascript"> 
    ...
    function postMessages(){
          ...
          if(messages != ""){ 
             $.ajax({
                  url : "support.php", // on donne l'URL du fichier de traitement
    ...
    function getMessages(){
      ...
      requeteAjax.open("GET", "support.php?op=charger");*/
      /*const requeteAjax = new XMLHttpRequest();
      requeteAjax.open('GET', 'handler.php?task=charger');
      ...
      $.ajax({
          url : "support.php?task=charger", // on donne l'URL du fichier de traitement
                            ...
                            <div class="app-page-title">
                                <div class="page-title-wrapper">
                                    <div class="page-title-heading">
                                        ...
                                        <div>  
                                            Votre support Partenaire CENTER - <?php  echo $correspondant ?>
                        ...
                        <div class="app-inner-layout__wrapper">
                            <div class="app-inner-layout__content card">
                                <div class="table-responsive">
                                        ...
                                        <div class="pane-left">
                                            ...
                                            <div class="avatar-icon-wrapper mr-2">
                                                <div class="avatar-icon avatar-icon-xl rounded">
                                                    <img width="82" src="assets/images/avatars/<?php echo $thisadmin["COC_ADMIN_file"] ?>" alt=""></div>
                                                    <?php 
                                                     if($status["COC_USER_status"]==1){ ?>
                                                        <div class="badge badge-bottom btn-shine badge-success badge-dot badge-dot-lg"></div>
                                                    <?php } ?>
                                            </div> 
                                            <h4 class="mb-0 text-nowrap"><?php echo $thisadmin["COC_ADMIN_prenom"]." ".$thisadmin["COC_ADMIN_nom"] ?></h4> 
                                        </div>
                                    ...
                                    <?php 
                                       // include("include/test_support.php");
                                     ?>
                                    ...
                                    <div class="chat-wrapper">
                                         <?php   
                                            foreach ($supports as  $value) { 
                                                ...
                                                if($value["typeexp"]!=$_SESSION['user']['type']){
                                         ?>
                            ...
                            <div class="app-inner-layout__sidebar card">
                                <div class="app-inner-layout__sidebar-header">
                                    <ul class="nav flex-column">
                                        ...
                                        <li class="nav-item-header nav-item">Mes interlocuteurs < <?php echo $_SESSION['correspondant'] ?> ></li>
                                    </ul>
                                </div>
                                <ul class="nav flex-column">
                                    <?php  
                                        if ($thisadmin) { 
                                            $admin=$thisadmin;
                                            $idexpAdmin=$thisadmin["COC_ADMIN_id"];
                                            $typeexpi=$thisadmin['COC_ADMIN_correspondant'];  
                                             $messagesAdmin=$dbh->query("SELECT messages FROM coc_support WHERE idexp='$idexpAdmin'  AND typeexp='$typeexpi' AND iddest='$iddest' ORDER BY date DESC LIMIT 0, 1")->fetch();
                                    ?>
                                        <li class="nav-item">
                                            <button   tabindex="0" onclick="document.location.href='support.php" class="dropdown-item ">
                                                        ...
                                                        <div class="widget-content-left mr-3">
                                                            <div class="avatar-icon-wrapper">
                                                                ...
                                                                <div class="avatar-icon">
                                                                    <img src="assets/images/avatars/<?php echo $admin['COC_ADMIN_file'] ?>" alt="">
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div class="widget-content-left">
                                                            <div class="widget-heading"><?php echo $admin["COC_ADMIN_prenom"]." ".$admin["COC_ADMIN_nom"] ?></div>
                                                            <div class="widget-subheading">
                                                                <?php  
                                                                    echo $messagesAdmin["messages"]; 
                                                                 ?>
                                                            </div>
                                                        </div>
                                                    </div>
